...
def probe_folder_structure(target_dir):
    with open("ae資料夾結構.txt", 'a', encoding='utf-8') as out:
        print(f"\n🔍 探測目標: {target_dir}\n" + "="*50)
        out.write(f"\n🔍 探測目標: {target_dir}\n" + "="*50 + "\n")
        
        if not os.path.exists(target_dir):
            print(f"找不到目錄: {target_dir}")
            out.write(f"找不到目錄: {target_dir}\n")
            return
                ...
                if ext == '.json':
                    try:
                        with open(file_path, 'r', encoding='utf-8') as json_file:
                            ...
                            out.write(f"{sub_indent}📄 {f}  -> [JSON 結構: {key_str}]\n")
                    except Exception as e:
                        print(f"{sub_indent}📄 {f}  -> [JSON 解析失敗: {e}]")
                        out.write(f"{sub_indent}📄 {f}  -> [JSON 解析失敗: {e}]\n")
                else:
...
if __name__ == "__main__":
    ...
    # 建議先測試 advanced_ae 
    TARGET = r"C:\Users\etyet\Downloads\All of Create - Aeronautics-v2.5_task_faf972e0_汉化补丁\resourcepacks\BBSMC汉化包\assets\schematicenergistics"
